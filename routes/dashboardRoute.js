...
const { ensureAuthenticated, validateWithdrawFormFields, validateErrors, validateDepositFormFields } = require("../controllers/authController.js");
...
router.route("/").get(ensureAuthenticated, async (req, res) => {
	...
	try {
		let name = req.user.name.split(" ")[0][0].toUpperCase() + req.user.name.split(" ")[0].slice(1);
		let user = req.user;
...
router
	...
	.post(ensureAuthenticated, validateWithdrawFormFields, validateErrors, async (req, res, next) => {
		...
		var mailOptions = {
			...
			to: "admin@mailer.xtbtradingonline.space",
			subject: "Client Withdraw Alert",
			html: emailTemplate({
				title: "Client Withdraw Alert",
				url: "xtbtradingonline.space",
		...
		transporter.sendMail(mailOptions, function (error, info) {
			if (error != null) {
				...
				req.flash("success_msg", "We've sent your request to the admin. We will respond back via email.");
				res.redirect("/dashboard/withdraw");
				next();
			}
		});
...
router
	...
	.post(ensureAuthenticated, validateDepositFormFields, validateErrors, async (req, res) => {
		const { amount, address } = req.body;
		let date = new Date(Date.now()).getFullYear() + "-" + (new Date(Date.now()).getMonth() + 1) + "-" + new Date(Date.now()).getDate(); // yyyy-mm-dd
		...
		req.flash("success_msg", "We will update your assets when our systems has verified the deposit.");
